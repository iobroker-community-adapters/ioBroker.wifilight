...
function CDevice(_name, _devices) {
    ...
    this.split = function (id, valOrObj, showName) {
        const ar = (id && id[0] === '.' ?
            id.substr(1) :
            dcs(deviceName, channelName, id)).split('.');

        const dName = deviceName, cName = channelName;
        if (ar.length === 3) {
            self.setDevice(ar.shift());
            self.setChannel(ar.shift());
            const ret = this.addEx(ar[0], valOrObj, showName);
            deviceName = dName;
            channelName = cName;
            return ret;
        }
    ...
    this.addEx = function (name, valOrObj, showName) {
        if (valOrObj == null) {
            return;
        }
        if (name.includes('.')) {
    ...
    this.set = function (id, newObj, showName) {
        ...
        if (!this.devices.objects[_id]) {
            return this.addEx(id, newObj, showName);
        }
